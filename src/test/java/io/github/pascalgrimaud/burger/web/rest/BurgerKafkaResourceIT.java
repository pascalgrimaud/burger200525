...
import io.github.pascalgrimaud.burger.config.KafkaProperties;
...
class BurgerKafkaResourceIT {
    ...
    void setup() {
        ...
        BurgerKafkaResource kafkaResource = new BurgerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/burger-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/burger-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
